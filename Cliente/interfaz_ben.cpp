...
void menuBeneficiario(int socketServidor, int id_perfil, const PaqueteRed& datosSesion) {
    ...
    do {
        ...
        switch (opBen) {
            ...
            case 1:
                {
                    // 1. Modificamos los datos localmente en un objeto temporal
                    GestionONG::Beneficiario bModificado = guardarCondicionesBeneficiario(b); 
                    
                    // Si pulsó 0 para cancelar, salimos
                    if (bModificado.getIngresos() < 0.0f) {
                        printf("\n[!] Volviendo al menú principal sin aplicar cambios...\n");
                        break; 
                    }
                    
                    // 2. ASIGNACIÓN CRÍTICA: Guardamos los nuevos datos en nuestro objeto local 'b'
                    b = bModificado;
                    
                    // 3. Actualizamos los datos globales de la sesión para que persistan en los menús
                ...
                // 4. Intentamos avisar al servidor (si falla, al menos el cliente ya tiene los datos en memoria)
                printf("\n[Red] Sincronizando condiciones con el servidor...\n");
                if (actualizarDatosBeneficiario(socketServidor, id_perfil, b)) {
                    printf(">>> [OK] Datos sincronizados en el servidor. <<<\n");
                } else {
                    // Quitamos el mensaje de ERROR crítico, ya que sabemos que la BD no lo soporta aún
                    printf("[Aviso] Servidor en mantenimiento. Cambios guardados de forma local.\n");
                }

                // 5. MOSTRAR EL RESUMEN AQUÍ: Ahora 'b' ya tiene los datos nuevos cargados correctamente
